...
//         ```
//
//  Step 3. Using the same starter code as in task 2, create a function called `animalCan` and within it, loop
//  over all the properties in both the bird object and its prototype - the animal object - using the for...in loop.
...
function logDairy() {
    for (const dair of Object.values(dairy)) {
        console.log(dair);
    }
}
...
function animalCan() {
    for (const promp in bird) {
        console.log(`${promp}: ${bird[promp]}`)
    }
}
